...
\usepackage[xcharter, slantedGreek, scaled=1.055, upint]{newtxmath}
...
\usepackage[textmd, semibold, scale=0.91]{plex-mono}  % To match the x-height of the Charter serif font
\usepackage[textmd, semibold, scale=0.91]{plex-sans}[2018/12/31]
...
\ifxetex
	% Do nothing.
\else
	\DisableLigatures[f]{family = {rm*, sf*, tt*}}
	% Disable the f* ligatures for the Plex fonts because the fonts do not provide sufficient support.
