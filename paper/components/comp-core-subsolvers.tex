...
and drop the subscript $t$ for brevity. %i.e. we write $M^{s} (s)$ instead of $M_{t}^{s} (x_t + s)$.
...
\begin{algorithm} %[H]
    ...
    \For{$\ell$=0,1,2,...}{
        ...
        $r_{\ell}\approx arg\,\min\limits_rM_{\ell}^r(x + r) \quad \text{s.t.} \ \lVert r \rVert \leq \Delta^{sub}_{\ell}$\\
