...
%The two definitions are summarized in \ref{eq:ball-metric}
...
In the subsequent discussion, we leave the dependence on the trust region center $x_t$ implicit and drop the subscript $t$ for brevity. %i.e. we write $M^{s} (s)$ instead of $M_{t}^{s} (x_t + s)$.  Model $M_t$ is based on the Taylor-series expansion of $f$ around $x_t$
...
In the unconstrained case, i.e. there are no bounds on the parameter space, GQTPAR determines an exact solution to the trust region
