...
\noindent With the reduced conjugate gradient step $r_l$ in hand, the active parameters in $\mathfrak{B}(s)$ are then set to their respective lower, upper, or fixed bounds, which yields the candidate step $s$ (line 6 of Algorithm \ref{algo:bntr}). The process is repeated until convergence criteria are met.
...
\begin{algorithm} %[H]
    ...
    \For{$\ell$=0,1,2,...}{
        ...
        $r_{\ell}\approx arg\,\min\limits_rM_{\ell}^r(r) \quad \text{s.t.} \ r \leq \Delta^{sub}$\\
