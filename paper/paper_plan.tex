...
To find a candidate step $s^{*}_t$, we solve for the optimum of the aggregate model:\comment[]{equation 2.10 in DFOLS paper imposes condition on the step side to }
\begin{align}
...
\begin{itemize}
    ...
    \item Vector Model:  linear models for the vector of residuals $\mathbf{f}(x)$:\comment[]{MP: Need to discuss indexing}
    \begin{align}
...
\begin{itemize}
    ...
    \item calculate number of unallocated function evals based on the number of points on the line search and batch size ($batch\_size-1-n_evals_line_search$) \comment[]{MP: WHY $batch\_size-1$ ?}
