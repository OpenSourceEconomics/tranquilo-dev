    ...
    \For{t=0,1,2,...}{
        ...
        Solve the surrogate problem: $s_t\approx\underset{{\Hquad\lVert s\rVert\leq\Delta_t^{region}}}{arg\,\min}M_t^s(s)$\label{algo-core-line:solve}\\
            ...
            \While{$|\mathcal{X}^{model}_t|>n^{target}$ and $\lVert s_t\rVert\leq s^{min}$}{
                Drop worst points: $\mathcal{X}_t^{reduced}=Drop(\mathcal{X}_t^{model}, n^{drop},\Delta_t^{region})$ and set $\mathcal{X}_t^{model}=\mathcal{X}_t^{reduced}$\label{algo-core-listing-improve-geo}\\
        ...
        \While{$\lVert s_t\rVert\leq s^{min}$}{
            Drop worst points: $\mathcal{X}_t^{reduced}=Drop(\mathcal{X}_t^{model}, n^{drop},\Delta_t^{region})$\label{algo-core-listing-improve-geo}\\
